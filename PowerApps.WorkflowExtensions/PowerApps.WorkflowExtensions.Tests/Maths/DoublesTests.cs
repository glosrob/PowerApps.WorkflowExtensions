...
namespace PowerApps.WorkfowExtensionsTests.Maths
{
    ...
    public class DoublesTests
    {
        ...
        public void Adds_Two_Numbers(double first, double second, double expected)
        {
            //Arrange
            ...
            //Act
            var result = fakedContext.ExecuteCodeActivity<AddDoubles>(inputs);

            //Assert
            Assert.Equal(expected, (double)result["Result"]);
        }
        ...
        public void Subtracts_Two_Numbers(double first, double second, double expected)
        {
            //Arrange
            var fakedContext = new XrmFakedContext();
            var inputs = new Dictionary<string, object>()
            ...
            //Act
            var result = fakedContext.ExecuteCodeActivity<SubtractDoubles>(inputs);

            //Assert
            Assert.Equal(expected, Math.Round((double)result["Result"], 2));
        }
        ...
        public void Multiplies_Two_Numbers(double first, double second, double expected)
        {
            //Arrange
            var fakedContext = new XrmFakedContext();
            var inputs = new Dictionary<string, object>()
            ...
            //Act
            var result = fakedContext.ExecuteCodeActivity<MultiplyDoubles>(inputs);
